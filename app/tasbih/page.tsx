...
export default function TasbihPage() {
    ...
    const [alerts, setAlerts] = useState<Record<string, boolean>>({ Fajr: true, Dhuhr: true, Asr: true, Maghrib: true, Isha: true });
    ...
    useEffect(() => {
        ...
        if (count > 0 && count % TARGETS[targetIndex] === 0 && !hasVibrated) {
            ...
            setHasVibrated(true);
        } else if (count % TARGETS[targetIndex] !== 0) { setHasVibrated(false); }
    ...
    const increment = () => { setCount(prev => prev + 1); if (window.navigator.vibrate) window.navigator.vibrate(30); };
    ...
    return (
        <div style={{ paddingTop: 16, paddingBottom: 100, minHeight: '100vh', background: bgPage, transition: 'background 0.3s' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', marginBottom: 20 }}>
                ...
                <h1 style={{ fontSize: 20, fontWeight: 800, color: textMain }}>{t.tasbihTitle}</h1>
                <button onClick={() => setShowSettings(!showSettings)} style={{ width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: 'pointer' }}>
                    <MoreVertical size={22} color={textMain} />
                </button>
            </div>
            ...
            <div style={{
                ...
                <div style={{ position: 'relative', width: 200, height: 200, marginBottom: 16 }}>
                    <svg width="200" height="200" style={{ position: 'absolute', top: 0, left: 0 }}>
                        {Array.from({ length: 60 }).map((_, i) => {
                            ...
                            return (
                                <line key={i}
                                    x1={100 + innerR * Math.cos(angle)} y1={100 + innerR * Math.sin(angle)}
                                    x2={100 + outerR * Math.cos(angle)} y2={100 + outerR * Math.sin(angle)}
                                    stroke={isMajor ? "#008D63" : (isDark ? "#2D5940" : "#C8E6D8")} strokeWidth={isMajor ? 2 : 1}
                                />
                            );
                        })}
                    </svg>
                    <svg width="200" height="200" style={{ position: 'absolute', top: 0, left: 0, transform: 'rotate(-90deg)' }}>
                        <circle cx="100" cy="100" r="90" fill="transparent" stroke={isDark ? '#2D3748' : '#E6F3EF'} strokeWidth="6" />
                        <circle cx="100" cy="100" r="90" fill="transparent" stroke="#008D63" strokeWidth="6"
                    ...
                    <div style={{
                        ...
                        borderRadius: '50%', background: bgCard,
                        boxShadow: isDark ? '0 2px 20px rgba(0,0,0,0.3)' : '0 2px 20px rgba(0,141,99,0.08)',
                        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                        transition: 'all 0.3s',
                    }}>
                        <p style={{ fontSize: 12, color: textMuted, fontWeight: 500, marginBottom: 2 }}>{currentDhikr.latin}</p>
                        <span style={{ fontSize: 48, fontWeight: 800, color: textMain, lineHeight: 1 }}>{count % currentTarget}</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
                            <span style={{ fontSize: 11, color: textMuted, fontWeight: 600 }}>Target: {currentTarget}</span>
                            <button onClick={reset} style={{ width: 18, height: 18, borderRadius: '50%', background: subtleBg, display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                                <RotateCcw size={9} color={textMuted} />
                            </button>
                        </div>
                    </div>
                    <div style={{ position: 'absolute', top: 2, left: '50%', transform: 'translateX(-50%)', width: 12, height: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <p style={{ fontSize: 11, color: textMuted, marginBottom: 16 }}>{currentDhikr.meaning}</p>
                <div style={{ fontSize: 11, color: textMuted, background: subtleBg, padding: '4px 12px', borderRadius: 20, fontWeight: 600, marginBottom: 4 }}>Total: {count}</div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, padding: '0 20px', marginBottom: 24 }}>
                <button onClick={increment} style={{
                    ...
                    background: isDark ? '#E8ECEF' : '#1A1F25', color: isDark ? '#1A1F25' : '#FFFFFF', fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer',
                    boxShadow: '0 4px 16px rgba(0,0,0,0.15)',
                }}>
                    <Fingerprint size={20} /> {t.tapToCount}
                </button>
                <button onClick={cycleTarget} style={{
                    width: 52, height: 52, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: bgCard, border: `1px solid ${borderColor}`, cursor: 'pointer',
            ...
            <div style={{ padding: '0 20px', marginBottom: 24 }}>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: textMain, marginBottom: 10 }}>{t.selectDhikr}</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {DHIKR_LIST.map((d, i) => (
                        <button key={d.latin} onClick={() => setDhikrIndex(i)} style={{
                            padding: '12px', borderRadius: 14, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer',
                            transition: 'all 0.2s', textAlign: 'left',
                            background: dhikrIndex === i ? (isDark ? '#1A2F26' : '#E6F3EF') : bgCard,
                            color: dhikrIndex === i ? '#008D63' : textMuted,
                            boxShadow: dhikrIndex === i ? '0 0 0 2px #008D63' : `0 0 0 1px ${borderColor}`,
                        }}>
            ...
            <div style={{ padding: '0 20px', marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <h3 style={{ fontSize: 18, fontWeight: 800, color: textMain }}>{t.nextPrayerLabel}</h3>
                ...
                {nextPrayer && (
                    <div style={{
                        ...
                        <div style={{ position: 'absolute', top: -20, right: -20, opacity: 0.12 }}><Moon size={100} /></div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, opacity: 0.8 }}><MapPin size={14} /> <span>Location: {location}</span></div>
                        ...
                        {["Fajr", "Dhuhr", "Asr", "Maghrib", "Isha"].map((p) => {
                            ...
                            return (
                                <div key={p} style={{
                                    ...
                                    background: isNext ? (isDark ? '#3D2700' : '#FFF3E0') : bgCard,
                                    border: isNext ? '2px solid #FF9800' : `1px solid ${borderColor}`,
                                    transition: 'all 0.3s',
                                }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                                        <div style={{
                                            width: 36, height: 36, borderRadius: '50%',
                                            background: isNext ? '#FF9800' : (isDark ? '#1A2F26' : '#E6F3EF'),
                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        }}>
                                            <div style={{ width: 8, height: 8, borderRadius: '50%', background: isNext ? 'white' : '#008D63' }} />
                                        </div>
                                        <div>
                                            <h4 style={{ fontSize: 15, fontWeight: 700, color: isNext ? '#E65100' : textMain }}>{p}</h4>
                                            <p style={{ fontSize: 12, color: textMuted }}>{timings[p]}</p>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        {isNext && <span style={{ fontSize: 10, fontWeight: 700, color: '#E65100', textTransform: 'uppercase', background: isDark ? '#5C3B00' : '#FFE0B2', padding: '2px 8px', borderRadius: 6 }}>NOW</span>}
                                        <button onClick={() => setAlerts(prev => ({ ...prev, [p]: !prev[p] }))} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>
                                            {alerts[p] ? <Volume2 size={18} color={isNext ? '#E65100' : '#008D63'} /> : <BellOff size={18} color={textMuted} />}
                                        </button>
                                    </div>
                                </div>
                            );
